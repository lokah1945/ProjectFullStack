...
export async function generateMetadata({
  ...
  return {
    ...generateSiteMetadata(site, { article }),
    alternates: {
...
interface LocaleArticlePageProps {
...
export default async function LocaleArticlePage({ params }: LocaleArticlePageProps) {
  ...
  const [article, adSlots, site, categories] = await Promise.all([
    fetchArticleBySlug(siteSlug, slug, locale).catch(() => null),
    fetchAdSlots().catch(() => []),
    fetchSiteBySlug(siteSlug).catch(() => null),
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-10">
            <div>
              ...
              <nav aria-label="Breadcrumb" className="text-sm text-gray-500 mb-6 flex items-center gap-2">
                <a href={`/${locale}`} className="hover:text-[var(--color-primary)] transition-colors">Home</a>
                <span aria-hidden>›</span>
                {article.category && (
                  <>
                    <a href={`/${locale}/category/${article.category.slug}`} className="hover:text-[var(--color-primary)] transition-colors">
              ...
              {article.category && (
                <a href={`/${locale}/category/${article.category.slug}`} className="category-badge mb-4 inline-block">
                  {article.category.name}
                </a>
              )}
                  ...
                  <div className="flex items-center gap-2">
                    {article.author.avatar?.url && (
                      <div className="relative w-8 h-8 rounded-full overflow-hidden">
                        <Image src={resolveImageUrl(article.author.avatar.url)} alt={article.author.name} fill className="object-cover" sizes="32px" />
                      </div>
                    )}
                ...
                {article.tags && article.tags.length > 0 && (
                  <div className="flex gap-2 flex-wrap">
                    {article.tags.map(tag => (
                      <span key={tag.documentId} className="text-xs bg-gray-100 text-gray-600 px-2 py-1 rounded-full">#{tag.name}</span>
                    ))}
                  </div>
                )}
              </div>
              ...
              {article.content && article.content.length > 0 ? (
                <AdInsertionEngine content={article.content} inArticleSlots={inArticleSlots} className="prose max-w-none" />
              ) : (
              ...
              {relatedArticles.length > 0 && (
                <section className="mt-12 pt-8 border-t border-gray-100">
                  <h2 className="text-xl font-display font-bold text-gray-900 mb-6">Related Articles</h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {relatedArticles.map(related => (
                      <a key={related.documentId} href={`/${locale}/article/${related.slug}`}
                        className="group flex gap-3 p-3 rounded-xl border border-gray-100 hover:border-gray-200 hover:bg-gray-50 transition-colors">
                        {related.coverImage?.url && (
                          <div className="relative w-20 h-16 flex-shrink-0 rounded-lg overflow-hidden bg-gray-100">
                            <Image src={resolveImageUrl(related.coverImage.url)} alt={related.title} fill className="object-cover" sizes="80px" />
                          </div>
                        )}
            ...
            <aside className="hidden lg:block">
              <div className="sticky top-20">
                <SidebarWidgets trendingArticles={trendingArticles} featuredArticles={featuredArticles} adSlots={adSlots} locale={locale} />
              </div>
            </aside>
          </div>
        </article>
